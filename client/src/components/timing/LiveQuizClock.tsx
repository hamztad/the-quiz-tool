...
interface LiveQuizClockProps {
  room: PublicRoomState;
}

export function LiveQuizClock({ room }: LiveQuizClockProps) {
  ...
  if (room.phase === 'lobby' && quizStartsAt) {
    return (
      <div
        className="mb-4 rounded-2xl border-2 border-violet-300/60 bg-gradient-to-r from-violet-50 to-cyan-50 px-4 py-4 text-center shadow-sm"
        role="timer"
        aria-live="polite"
        aria-atomic="true"
      >
  ...
  if ((room.phase === 'live' || room.settings.teamsLockedOut) && quizEndsAt) {
    return (
      <div
        className="mb-4 rounded-2xl border-2 border-amber-300/60 bg-gradient-to-r from-amber-50 to-orange-50 px-4 py-3 shadow-sm"
        role="timer"
        aria-live="polite"
        aria-atomic="true"
      >
